...
class Reports extends Secure_area
{
	...
	function summary_payments($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['payment_type'], $row['count'], to_currency($row['payment_amount']),$row['payment_amount']);
		}
	...
	function inventory_summary_input()
	{
		...
		$data['item_count'] = $model->getItemCountDropdownArray();
                $data['supplier'] = $model->getAllsupplier();
	...
	function inventory_summary($export_excel=0, $location_id = 'all', $item_count = 'all',$supplierid="all")
	{
		$this->load->model('reports/Inventory_summary');
		$model = $this->Inventory_summary;
		$tabular_data = array();
		$report_data = $model->getData(array('location_id'=>$location_id,'item_count'=>$item_count,'supplierid'=>$supplierid));
		foreach($report_data as $row)
		{
