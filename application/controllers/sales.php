...
class Sales extends Secure_area {
    ...
    function add() {
        ...
        $returnallowed = $this->Employee->has_grant('sales_return', $person_info->person_id);

        if ($this->sale_lib->out_of_stock_no($item_id_or_number_or_item_kit_or_receipt, $item_location)) {
            $data['warning'] = $this->lang->line('sales_quantity_less_than_zero');
        } elseif ($mode == "return" && $returnallowed == false) {
            $data['warning'] = "You are not allowed for taking return";
        } else {


            if ($mode == 'return' && $this->sale_lib->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
                $data['error'] = "error to return item"; //$this->sale_lib->return_entire_sale($item_id_or_number_or_item_kit_or_receipt);
            ...
            } else if (!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $quantity, $item_location, 6)) {
    ...
    function edit_item($line) {
        ...
        if ($this->sale_lib->out_of_stock($this->sale_lib->get_item_id($line), $item_location)) {
            $data['warning'] = $this->lang->line('sales_quantity_less_than_zero');
        } elseif ($quantity < 0 && $returnallowed == false) {
            $data['warning'] = "Do not use nagetive value";
        } else {
            if ($this->form_validation->run() != FALSE) {
    ...
    function complete() {
        ...
        //Added by me
        //$data['current_score']=$this->sale_lib->get_subtotal(TRUE)*0.01;
    ...
    function delete($sale_id = -1, $update_inventory = TRUE) {
        ...
        echo json_encode(array('error' => false, 'message' => 'Deletion not allowed'));
    ...
    private function _reload($data = array()) {
        ...
        $data['grantreturn'] = $this->Employee->has_grant('return', $person_info->person_id);
        $data['comment'] = $this->sale_lib->get_comment();
        $data['email_receipt'] = $this->sale_lib->get_email_receipt();
        $data['payments_total'] = $this->sale_lib->get_payments_total();
        $data['amount_due'] = $this->sale_lib->get_amount_due();
        $data['payments'] = $this->sale_lib->get_payments();
        $data['totaldiscount'] = $this->sale_lib->get_discount();
