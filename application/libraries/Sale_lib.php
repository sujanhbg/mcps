...
class Sale_lib {
    ...
    function get_discount_percent($itemid) {
        return 55;
    }
    ...
    function add_item($item_id, $quantity = 1, $item_location, $discount = 0, $price = null, $description = null, $serialnumber = null) {
        ...
        foreach ($this->CI->db->query("SELECT * FROM ospos_items_taxes WHERE item_id=" . $item_id)->result() as $taxress) {
            $tax_percent = $taxress->percent;
        }
        ...
        //added items discount
        $discounted_price = $item_info->custom10;
        if (!$discounted_price || $discounted_price == 0) {
            $discount = 0;
        } else {
            $discount = (($price - $discounted_price) / $price) * 100; //number_format((float)(($price-$discounted_price)/$price)*100, 0, '.', '');
        }
        ...
        if (!$itemalreadyinsale || $item_info->is_serialized) {
            $item = array(($insertkey) =>
                array(
                    ...
                    'price' => $price,
                    'total' => $total, 'cost_price' => $costprice,
    ...
    function out_of_stock_no($item_id, $item_location) {
        //make sure item exists
        if ($this->validate_item($item_id) == false) {
            return false;
        }
