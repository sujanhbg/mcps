...
class Inventory_summary extends Report
{
	...
	public function getData(array $inputs)
	{
		...
		elseif($inputs['item_count'] == 'more_than_zero')
		{
			$this->db->where('quantity >', 0);
                ...
                if($inputs['supplierid'] == 'all')
                {
                    
                }elseif($inputs['supplierid']=='0') {
                    $this->db->where('items.supplier_id', null);
                }else{
                    $this->db->where('items.supplier_id', $inputs['supplierid']);
	...
	public function getItemCountDropdownArray()
	{
        ...
        $query = $this->db->query('SELECT person_id,company_name FROM ospos_suppliers');
